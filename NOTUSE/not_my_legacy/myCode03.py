...
if __name__ == '__main__':
    ...
    for i in range(max_epoch * 3):
        ...
        keep_ratio = 0.5  # 낮은 에너지 회로 중 상위 50%만 학습에 사용
        K = int(train_size * keep_ratio)
        topk_inds = np.argsort(train_seq_en.flatten())[:K]

        tokens = torch.from_numpy(train_token_seq[topk_inds])
        energies = torch.from_numpy(train_seq_en[topk_inds])
        train_inds = np.arange(K)
