...
from data import data_load_and_process, get_single_data
...
class GPTQE(GPT):
    ...
    def calculate_loss(self, tokens, energies, margin=0.3):
...
def get_sequence_energies(op_seq, X1, X2, Y, margin=0.3):
    energies = []
    for ops in op_seq:
        probs = fidelity_circuit(X1, X2, ops)
        fidelity = probs[0]
        value = 1.0 - torch.abs(fidelity - Y)
        energy = torch.clamp(margin - value, min=0)
...
if __name__ == '__main__':
    ...
    for i in range(max_epoch):
        X1, X2, Y = get_single_data(X_train, Y_train)
