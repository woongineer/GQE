...
class BiasNet(nn.Module):
    def __init__(self, num_of_bias, num_of_feature):
        ...
        self.head = nn.Sequential(
            nn.Linear(num_of_feature * 4, num_of_feature * 4), nn.ReLU(),
    ...
    def forward(self, x1, x2):
        ...
        h12 = torch.cat([x1, x2, x_sum, x_diff], dim=1)
        h21 = torch.cat([x2, x1, x_sum, x_diff], dim=1)
...
def run_compare(data_x, data_y, num_qubits, n_layer, batch_size, epoch, good_circuits, bad_circuits, rand_circuits, ave_len):
    ...
    opts["zz_nqe"] = optim.SGD(nets["zz_nqe"].parameters(), lr=0.01)
...
if __name__ == "__main__":
    ...
    n_layer = 1
    epoch = 4  #100
    averaging_length = 1  #10
    num_cpus = 2  #16
    repeat = 2  #16
    html_filename = f"prefactor_tune/2xbias_epoch{epoch}"
