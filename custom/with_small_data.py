...
class GPTQE(GPT):
    def __init__(self, config):
        ...
        self.logit_to_energy = torch.nn.Sequential(
    ...
    def calculate_loss(self, tokens, energies):
        ...
        cumsum_logits = torch.cumsum(next_token_logits, dim=1)[:, -1:]  # (B, 1)
        pred_energies = self.logit_to_energy(cumsum_logits)
        # match cumulative logits to subsequence energies
        # loss = torch.mean(torch.square(cumsum_logits - energies))
        loss = F.mse_loss(pred_energies, energies)
        return loss
    ...
    def generate(self, n_sequences, max_new_tokens, temperature=1.0, device="cpu"):
        ...
        pred_energies = self.logit_to_energy(total_logits)
        return idx, pred_energies
