...
def data_load_and_process(dataset="mnist", reduction_sz: int = 4, train_len=400, test_len=100):
    ...
    if dataset == "mnist":
        ...
        X_red = PCA(reduction_sz, random_state=0).fit_transform(X_std)
        ...
        for x in X_red:
            ...
            if xmax - xmin < 1e-12:
                ...
                X_scaled.append((x - xmin) * (2 * np.pi / (xmax - xmin)))
        ...
        om = fetch_openml(name="yeast", version=4, as_frame=True)
        X = om.data.to_numpy(dtype=np.float64)
        y_raw = om.target.to_numpy()
